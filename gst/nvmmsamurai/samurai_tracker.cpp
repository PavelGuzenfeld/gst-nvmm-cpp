              ...
              size_t bytes, cudaStream_t stream)
{
    ...
    if (FILE *f = std::fopen(path.c_str(), "wb")) {
        ...
        std::fclose(f);
        GST_INFO("dumped %s (%zu bytes)", path.c_str(), bytes);
    }
}
}  // namespace
...
bool SamuraiTracker::Impl::seed_cond_frame(const TrackBox &box, std::string &err)
{
    ...
    GST_INFO("seed cond frame: best=%d iou=%.3f obj=%.3f box(%.0f,%.0f %.0fx%.0f) maskpx=%s",
...
bool SamuraiTracker::Impl::track_frame(TrackResult &out, std::string &err)
{
    ...
    GST_DEBUG("track f=%ld sel=%d obj=%.2f stable=%d box(%.0f,%.0f %.0fx%.0f)%s",
